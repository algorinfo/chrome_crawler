...
def test_crawl_chrome():
    data = asdict(CHROME)
    r = httpx.post(f"{SERVICE_URL}/v7/chrome", json=data, headers=HEADERS, timeout=120)
...
def test_crawl_axios():
    data = asdict(AXIOS)
    r = httpx.post(f"{SERVICE_URL}/v7/axios", json=data, headers=HEADERS, timeout=120)
    data = r.json()
    jdata = json.loads(r.content) 
...
def test_crawl_search_duck():
    ...
    r = httpx.post(f"{SERVICE_URL}/v7/duckduckgo", json=data, headers=HEADERS, timeout=220)
    data = r.json()
    assert r.status_code == 200
...
def test_crawl_search_google():
    ...
    r = httpx.post(f"{SERVICE_URL}/v7/google", json=data, headers=HEADERS, timeout=220)
    data = r.json()
    assert r.status_code == 200
    assert len(data["links"]) > 0
