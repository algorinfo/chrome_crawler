...
const { crawlPage, parseCrawlPage } =  require("../browser6.js");
...
router.post("/axios", protected, async (ctx, next) => {
  ...
  try{
    ...
    const rspAx = await axios.get(options.url, axiosOpts);
...
router.post("/chrome", protected, async (ctx, next) => {
  ...
  try{
    ...
    rsp = await crawlPage(task, COOKIES_PATH)
...
router.post("/duckduckgo", protected, async (ctx, next) => {
  ...
  try{
    ...
    rsp = await crawlDuckGo(task, COOKIES_PATH)
    statusCode = rsp["status"]
  } catch(e){
    statusCode = 500
    rsp["error"] =  e
...
router.post("/google", protected, async (ctx, next) => {
  ...
  try{
    ...
    rsp = await crawlGoogle(task, COOKIES_PATH)
  } catch(e){
    statusCode = 500
    rsp["error"] =  e
    console.error(e)
